...
**职责（Task）**：借鉴公司 Aladdin 规格化编码流水线思路，**自研并落地一套面向 Android→鸿蒙（ArkTS/ArkUI）的 AI 辅助转换方案（A2H）**，在保证 UI 还原度与可编译的前提下显著提升转换效率，并把转换经验沉淀为团队资产。
...
- **复用 DcarHarmony + DcarKMP 双仓收敛翻译面**：业务逻辑经 KMP 共享后，AI 只需翻译 UI 与平台适配层，**大幅降低幻觉与编译失败率**（架构决定 AI 上限）。
